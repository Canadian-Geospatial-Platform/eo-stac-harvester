...
source='eodms'
lastrun = 'lastRun_eodms_sentinel1.txt'
...
def lambda_handler(event, context):
    ...
    print(f'Working dirctory when enter the lambda_handler {os.getcwd()}')
    os.chdir('/tmp')    
    if not os.path.exists(os.path.join('mydir')):
        os.makedirs('mydir')
        
    print(f'Working dirctory after reset the dir to /tmp, also before creating response_root {os.getcwd()}')
    # Before harvesting the STAC api, we check the root api connectivity first   
    try: 
        print(f'Enetring the try-except to request api_root: {api_root}')
    ...
    print(f'Working dirctory when the response_root.stastus_code  {os.getcwd()}')
    ...
    if response_root.status_code == 200:
        ...
        e = delete_stac_s3(bucket_geojson=geocore_to_parquet_bucket_name, bucket_template=geocore_template_bucket_name, filename=lastrun)
        if e != None: 
            error_msg += e
        end_time_delete = time.time()
        print(f"Deleting the previous harvester took {(end_time_delete - start_time_delete)/60:.2f} minutes ({(end_time_delete - start_time_delete):.2f} seconds) to run.")
        # Create a new log file and write into log file for each sucessfull harvest 
        print('Creating a new lastRun_eodms_sentinel1.txt')
        with open('/tmp/lastRun_eodms_sentinel1.txt', 'w') as f:
            # Catalog Level     
           ...
           #TBD using first collection bounding box could cause potential issues when collections have different extent, a solution is required. 
            ...
            # Collection mapping, Sentinal-1 only 
            collection_sentinal_list = [entry for entry in collection_data_list if entry.get('id') == 'sentinel-1']
            collection_sentinal_dict =collection_sentinal_list[0]
            #print(f'Type of the sentinal1 collection dictionary is {type(collection_sentinal_dict)}')

            # Reload the null GeoCore template 
            geocore_features_dict = get_geocore_template(geocore_template_bucket_name,geocore_template_name) 
            coll_extent = collection_sentinal_dict.get('extent')
            ...
            coll_properties_dict = coll_to_features_properties(coll_dict=collection_sentinal_dict, params=params, geocore_features_dict=geocore_features_dict)
            ...
            coll_id = collection_sentinal_dict.get('id')
            ...
            if msg == True: 
                print(f'Mapping Collection: {coll_id}. Finished and Uploaded the collection to bucket: {geocore_to_parquet_bucket_name}')
                f.write(f"{coll_name}\n")    
            
            #Item level   
            """
            # Print the total count of itmes in the collection 
            pages = search_pages_get(url=api_root +'/collections/sentinel-1/items')
            print(f'Type of pages are {type(pages)}')
            print(pages[0])
            item_count = 0 
            print(len(pages))
            print(type(pages))
            ...
            print(f"Search the items and make the list of pages took {(end_time_search - start_time_search)/60:.2f} minutes ({(end_time_search - start_time_search):.2f} seconds) to run.")
       
            item_count = 0 
            # For testing, mapping the first page only 
            pages = pages[0:2]
        ...
        f.close()
        msg = upload_file_s3(filename='lastRun_eodms_sentinel1.txt', bucket=geocore_template_bucket_name, json_data = None, object_name=None)
        if msg == True: 
            print(f'Finished mapping the EODMS Sentinal datacube and uploaded the lastRun.txt to bucket: {geocore_template_name}')   
    else:
